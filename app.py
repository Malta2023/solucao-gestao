...
def extrair_dados_pdf(pdf_file):
    try:
        ...
        # 2) fallback (modelo antigo do ObraGestor)
        dados = {"tipo": None}
        text_up = text.upper()
...
menu = st.sidebar.radio(
    "Navegação", ["📊 Dashboard", "🏗️ Gestão de Obras", "👥 Clientes", "📥 Importar/Exportar"]
)
...
if menu == "📊 Dashboard":
    ...
    with c1:
        ...
        if not df_obras.empty and "Data_Visita" in df_obras.columns:
            proximas = (
        ...
        if not proximas.empty:
            st.dataframe(proximas[["Cliente", "Data_Visita", "Status"]], use_container_width=True)
        else:
    ...
    if df_clientes.empty:
        ...
        if cli_selecionado:
            ...
            with st.form("form_obra_detalhe"):
                ...
                desc = st.text_area("Descrição do serviço", value=str(obra_atual.get("Descricao", "")), height=130)
...
# ---------- IMPORTAR ----------
    ...
    with c2:
        ...
        st.download_button(
            "Baixar obras (CSV)",
            df_obras.to_csv(index=False).encode("utf-8"),
            "obras_backup.csv",
            "text/csv",
        )
